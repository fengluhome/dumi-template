...
describe('Cascader', () => {
  ...
  it('should render not found content', () => {
    ...
    wrapper
      .find('input')
      .simulate('change', { target: { value: '__notfoundkeyword__' } });
  ...
  it('can use fieldNames', () => {
    ...
    wrapper
      .instance()
      .handleChange(['zhejiang', 'hangzhou', 'xihu'], customerOptions);
    expect(
      wrapper
  ...
  it('should warning if not find `value` in `options`', () => {
    ...
    mount(
      <Cascader
        options={[{ label: 'a', value: 'a', children: [{ label: 'b' }] }]}
      />,
    );
    expect(errorSpy).toHaveBeenCalledWith(
  ...
  it('can be selected when showSearch', () => {
    const onChange = jest.fn();
    const wrapper = mount(
      <Cascader options={options} onChange={onChange} showSearch />,
    );
    wrapper.find('input').simulate('click');
