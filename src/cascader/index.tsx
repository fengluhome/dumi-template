...
function defaultFilterOption(
  ...
  return path.some(
    option => (option[names.label] as string).indexOf(inputValue) > -1,
  );
}
...
function warningValueNotExist(
  list: CascaderOptionType[],
  fieldNames: FieldNamesType = {},
) {
  (list || []).forEach(item => {
    const valueFieldName = fieldNames.value || 'value';
    devWarning(
      valueFieldName in item,
      'Cascader',
...
class Cascader extends React.Component<CascaderProps, CascaderState> {
  ...
  getLabel() {
    const {
      options,
      displayRender = defaultDisplayRender as Function,
    } = this.props;
    ...
    const selectedOptions: CascaderOptionType[] = arrayTreeFilter(
      ...
      (o: CascaderOptionType, level: number) =>
        o[names.value] === unwrappedValue[level],
      { childrenKeyName: names.children },
    );
    const label = selectedOptions.length
      ? selectedOptions.map(o => o[names.label])
      : value;
  ...
  generateFilteredOptions(
    ...
    if (limit > 0) {
      ...
      filtered = flattenOptions.filter(path =>
        filter(this.state.inputValue, path, names),
      );
    }
    ...
    if (filtered.length > 0) {
      ...
      const field =
        names.value === names.label ? keepFilteredValueField : names.value;
  ...
  renderCascader = (
    ...
    <SizeContext.Consumer>
      {size => {
        ...
        const isNotFound =
          (options || []).length === 1 && options[0].isEmptyNode;
        if (isNotFound) {
          dropdownMenuColumnStyle.height = 'auto'; // Height of one row.
        }
        // The default value of `matchInputWidth` is `true`
        const resultListMatchInputWidth =
          (showSearch as ShowSearchType).matchInputWidth !== false;
        if (
          resultListMatchInputWidth &&
          (state.inputValue || isNotFound) &&
          this.input
        ) {
          ...
          inputIcon = replaceElement(
            ...
            () => ({
              className: classNames({
                [(suffixIcon as any).props.className!]: (suffixIcon as any)
                  .props.className,
                [`${prefixCls}-picker-arrow`]: true,
              }),
            }),
        ...
        const getPopupContainer =
          props.getPopupContainer || getContextPopupContainer;
        const rest = omit(
          props as typeof props & { inputIcon: any; loadingIcon: any },
          ['inputIcon', 'expandIcon', 'loadingIcon', 'bordered', 'className'],
        );
        const rcCascaderPopupClassName = classNames(popupClassName, {
          [`${prefixCls}-menu-${direction}`]: direction === 'rtl',
          [`${prefixCls}-menu-empty`]:
            options.length === 1 &&
            options[0].value === 'ANT_CASCADER_NOT_FOUND',
        });
    ...
    return (
      <ConfigConsumer>
        {(configArgument: ConfigConsumerProps) => (
          <LocaleReceiver>
            {locale => this.renderCascader(configArgument, locale)}
          </LocaleReceiver>
        )}
      </ConfigConsumer>
    );
