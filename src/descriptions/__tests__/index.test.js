...
describe('Descriptions', () => {
  ...
  it('when typeof column is object', () => {
    ...
    expect(
      wrapper.find('td').reduce((total, td) => total + td.props().colSpan, 0),
    ).toBe(8);
    wrapper.unmount();
  });
  ...
  it('number value should render correct', () => {
    ...
    expect(
      wrapper.find('th').hasClass('ant-descriptions-item-label'),
    ).toBeTruthy();
    expect(
      wrapper.find('td').hasClass('ant-descriptions-item-content'),
    ).toBeTruthy();
  });
